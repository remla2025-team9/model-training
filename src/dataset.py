...
def move_output_to_vectorizers(output_dir="output", target_dir="vectorizers"):
    ...
    if not os.path.exists(output_dir):
        logger.warning(
            f"Output directory '{output_dir}' does not exist, skipping move."
        )
        return
    ...
    try:
        ...
        logger.warning(
            f"Could not remove output directory '{output_dir}', it may not be empty."
        )
...
def load_and_generate_features(raw_data_path, text_col, label_col, sep="\t", quoting=3):
    ...
    logger.info(
        f"Generating features from text column '{text_col}' using external 'preprocess'"
    )
    ...
    logger.info(
        f"Generated features of shape {corpus_features.shape} and {len(labels)} labels."
    )
...
def main(args, test_size=0.2):
    ...
    logger.info(
        f"Train features shape: {X_train.shape}, Test features shape: {X_test.shape}"
    )
    ...
    pd.DataFrame(y_train, columns=["label"]).to_csv(
        config.TRAIN_LABELS_FILE, index=False
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Load raw data, generate features, split, and save."
    )
    parser.add_argument(
