...
def main(args):
    ...
    try:
        ...
        logger.error(
            "MemoryError converting sparse features to dense. Consider alternatives."
        )
        raise
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Train a sentiment analysis classifier.'
    )
    parser.add_argument(
        '--model_type',
        choices=['nb', 'logistic'],
    ...
    if not config.TRAIN_FEATURES_FILE.exists() or not config.TRAIN_LABELS_FILE.exists():
        logger.warning(
            "Processed train data not found. "
            "Please run 'python -m sentiment_src.dataset' first."
        )
    else:
        main(parsed_args)
